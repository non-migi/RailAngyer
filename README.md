...
| [files/23_courses_v5.sql](files/23_courses_v5.sql) | モスクワ環状線・大環状線・北京10号線の投入（**未適用**） |
| [files/24_courses_v6.sql](files/24_courses_v6.sql) | 大阪環状線・ニューヨーク7号線・ロサンゼルスEラインの投入（**未適用**） |
...
- コースは14本。日本7本（南北線・東西線・東豊線・山手線・札幌市電・阪急京都本線・
  **大阪環状線**）と**海外7本**（ベルリン Ringbahn・ロンドン サークル線・
  モスクワ環状線・モスクワ大環状線・北京地下鉄10号線・
  **ニューヨーク7号線**・**ロサンゼルス Eライン**）。設定から切り替えられる。
  **山手線と市電は一周でき、内回り／外回りを選べる**（予定でも一周を決められる）
- コースは **国 → 都道府県 → 路線** の順にたどって選ぶ。**日本の外も入っている**（アメリカ・イギリス・ドイツ・ロシア・中国）。
...
- 地図は**実際に歩いた跡**を描き、**50mごとに速さで色を分ける**。
  これから行く線は**道路沿い**に引く（取れなければ直線）。お題のある駅にはピンが立つ
- **写真はどこからでも見られる**（ホーム＞写真）。地図のピンにも写真がそのまま出る
- **歩いている間も全体マップを見られる**。ターンの画面から盤面へ抜けられ、戻る口が出る
- 地図のお題は**文字で読める**（先頭だけ。全文は駅を押す）
- ふりかえりに**全体の地図・歩いた距離・お題ごとの写真**を出す
- **遊び方**を初回に出し、設定からいつでも読み返せる。利用規約も同梱
- 設定から**開発者を応援**できる（¥300/¥600/¥1000）。**買っても何も起こらない**。
  商品の登録と有料App契約は未了のため、**まだ売れない**
- 設定＞**データの出典**で国土数値情報・OpenStreetMap（ODbL）・MapKit を表示
...
- **8言語**に対応中（日本語・英語・ドイツ語・スペイン語・フランス語・韓国語・
  ロシア語・簡体字中国語）。**設定＞言語からアプリ内で切り替えられる**（再起動不要）。
  訳の無い文字列は日本語のまま出る
